...
decode() {
  ENCFS6_CONFIG="$ENCFS6_CONFIG" "$ENCFS_CTL_BIN" decode . "${1:?A path to decode is required}"
}

encode() {
  ENCFS6_CONFIG="$ENCFS6_CONFIG" "$ENCFS_CTL_BIN" encode . "${1:?A path to encode is required}"
}
